...
import type { Assertion, Outcome } from "./table.js";
...
export function returns<Context, Result>(
  expected: Result,
...
export function hasError<Context, Result>(
  expected: DomainErrorExpectation,
): Assertion<Context, Result> {
  return {
    ...
    check: (_ctx, result) => {
      ...
      if (!result.ok) {
        const comparableError = result.error instanceof Error
        ...
        if (message !== undefined) {
          const actualMessage = result.error instanceof Error
...
export function hasObserved<Context, Result>(
  key: string,
  expected: unknown,
): Assertion<Context, Result> {
  return {
...
export function hasCalls<Context, Result>(
  key: string,
  expected: unknown,
): Assertion<Context, Result> {
  return {
...
export function hasEvents<Context, Result>(
  key: string,
  expected: unknown,
): Assertion<Context, Result> {
  return {
