...
import { ApplicationError, type AgentSessionRepository, type MuximodApplication, type PaneRepository, type WorkspaceRepository, WorkspaceCrud } from "@muximo/application";
import { normalizeAgentSessionName, paneKindForCommand, type AgentBackend, type AgentSessionRecord, type PaneRecord, type PaneState, type WorkspaceRecord } from "@muximo/domain";
import type { CreatePaneRequest, PaneSummary, TerminalEndpoint, TmuxSession } from "@muximo/protocol";
import { buildMuximoShellCommand, configureManagedTmuxSession, resolveMuximoCommand, TmuxAdapter, type TmuxPane, type TmuxLiveSnapshot } from "../tmux.js";
...
async function createSession(
  ...
  let created = false;
  try {
    const binary = resolveMuximoCommand();
    tmux.createSession(input.name, cwd, buildMuximoShellCommand(binary, {
      MUXIMOD_MANAGED_SESSION_ID: managedSessionId,
      MUXIMOD_MANAGED_SESSION_NAME: input.name,
    }));
    created = true;
...
async function syncPanes(
  ...
  const managedMuximoCommand = resolveMuximoCommand();
  ...
  for (const tmuxPane of live.panes) {
    ...
    const adoptedSession = sessionCandidate
      ...
      && isManagedMuximoCommand(tmuxPane.command, sessionCandidate.backend, managedMuximoCommand)
      ? sessionCandidate
      : undefined;
    const staleAgentMetadata = tmuxPane.muximodKind === "agent"
      && Boolean(tmuxPane.muximodSessionId)
      && !adoptedSession;
    if (tmuxPane.muximodSessionId && !adoptedSession) {
      if (tmuxPane.muximodExecutionId) agentStatus.delete(agentStatusKey(tmuxPane.muximodSessionId, tmuxPane.muximodExecutionId));
      try {
        const cleared = tmux.clearAgentExecutionMetadata(tmuxPane.paneId, tmuxPane.muximodExecutionId ?? "");
...
function isManagedMuximoCommand(command: string, backend: AgentBackend, agentCommand: string): boolean {
  const executable = command.trim().split(/\s+/, 1)[0]?.split("/").at(-1)?.toLowerCase();
  const configuredMuximo = (process.env.MUXIMOD_MUXIMO_COMMAND ?? "muximo").trim().split(/\s+/, 1)[0]?.split("/").at(-1)?.toLowerCase();
