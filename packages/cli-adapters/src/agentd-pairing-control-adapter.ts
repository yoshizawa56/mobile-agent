...
/**
 * Unix-socket adapter for agentd's private pairing control protocol.
 *
 ...
 */
export class AgentdPairingControlAdapter implements PairingControlPort {
  ...
  private constructor(private readonly socket: Socket) {
    socket.on("error", (error) => {
      this.socketError = new PairingControlError(`agentd control socket failed: ${error.message}`, "control_socket_error");
    });
    this.reader = createInterface({ input: socket, crlfDelay: Infinity });
    this.responses = this.reader[Symbol.asyncIterator]();
  }

  public static async connect(socketPath: string): Promise<AgentdPairingControlAdapter> {
    const socket = await connectControlSocket(socketPath);
    return new AgentdPairingControlAdapter(socket);
  }
  ...
  private async request(request: AgentdControlRequest): Promise<AgentdControlResponse> {
    this.socket.write(`${JSON.stringify(agentdControlRequestSchema.parse(request))}\n`);
    const response = await this.nextResponse();
    if (response.type === "error") throw controlError(response);
    return response;
  }

  private async nextResponse(): Promise<AgentdControlResponse> {
    if (this.socketError) throw this.socketError;
    let next: IteratorResult<string>;
    try {
      next = await this.responses.next();
    } catch (error) {
      throw new PairingControlError(`agentd control socket failed: ${error instanceof Error ? error.message : String(error)}`, "control_socket_error");
    }
    if (this.socketError) throw this.socketError;
    if (next.done) throw new PairingControlError("agentd control socket closed before pairing completed", "control_socket_closed");

    let raw: unknown;
    try {
      raw = JSON.parse(next.value) as unknown;
    } catch {
      throw new PairingControlError("agentd control socket returned invalid JSON", "invalid_control_response");
    }
    const parsed = agentdControlResponseSchema.safeParse(raw);
    if (!parsed.success) throw new PairingControlError("agentd control socket returned an invalid response", "invalid_control_response");
    return parsed.data;
  }
}
...
function connectControlSocket(path: string): Promise<Socket> {
  return new Promise((resolve, reject) => {
    ...
    const onError = (error: Error) => reject(new PairingControlError(`could not connect to agentd control socket: ${error.message}`, "control_socket_connect_failed"));
    socket.once("connect", () => {
...
function controlError(response: Extract<AgentdControlResponse, { type: "error" }>): PairingControlError {
  return new PairingControlError(`${response.code}: ${response.message}`, response.code);
}

function unexpectedResponse(expected: string, received: AgentdControlResponse["type"]): PairingControlError {
