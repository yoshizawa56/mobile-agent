...
export function createMuximodApplication(resources: MuximodApplicationResources): MuximodApplicationRuntime {
  const { host, paneRepository, agentSessionRepository, workspaceCatalog, viewportManager, workspaceRepository, workspaceCrud } = resources;
  ...
  return {
    ...
    sessions: {
      ...
      create: (input) => createSession(input, host, paneRepository, agentSessionRepository, workspaceCatalog, agentStatus),
    },
    panes: {
      list: (sessionName) => listCurrentPanes(host, paneRepository, agentSessionRepository, agentStatus, sessionName),
      create: (input, workspace) => createPane(input, host, paneRepository, agentSessionRepository, viewportManager, workspaceCatalog, agentStatus, workspace),
    },
    ...
    adoptAgentSession: (request) => adoptAgentSession(host, paneRepository, agentSessionRepository, agentStatus, request),
    observeAgentSession: (request) => observeAgentSession(host, paneRepository, agentSessionRepository, agentStatus, request),
    releaseAgentSession: (request) => releaseAgentSession(host, paneRepository, agentSessionRepository, agentStatus, request),
  };
}
...
function resolvePaneKind(tmuxPane: MuximodPaneSnapshot, existing: PaneRecord | undefined, adopted: boolean, staleAgentMetadata: boolean): PaneRecord["kind"] {
...
function readUnmanagedAgentObservation(host: MuximodHostPort, pane: MuximodPaneSnapshot, fallback: PaneState): AgentStatusObservation {
  try {
