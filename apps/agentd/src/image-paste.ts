 ...
 * application, which agent CLIs parse to attach the image. agentd reproduces
...
export function createImagePaster(options: ImagePasterOptions): (input: ImagePasteInput) => ImagePasteResult {
  ...
  return (input) => {
    ...
    const bufferName = `agentd-paste-${randomBytes(6).toString("hex")}`;
...
function stageTempImage(input: ImagePasteInput, tempDir: string): string {
  const filePath = join(tempDir, `agentd-paste-${randomBytes(8).toString("hex")}${extensionForMimeType(input.mimeType)}`);
