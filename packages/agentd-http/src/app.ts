...
export class AgentdHttpError extends Error {
  public constructor(
    public readonly status: AgentdHttpStatus,
...
export function createAgentdApp(deps: AgentdHttpDependencies) {
  ...
  return new Hono<AppEnv>()
    ...
    .get("/health", (c) => {
      if (deps.isReady && !deps.isReady()) throw new AgentdHttpError(503, "agentd_unavailable", "agentd is still starting");
      return c.json(agentdHealthSchema.parse({ ok: true, service: "agentd", protocolVersion: 1 }));
    })
    ...
    .get("/auth/v1/pairings/:pairingId", validate("param", pairingParamsSchema), (c) => {
      ...
      if (!claimToken) throw new AgentdHttpError(401, "claim_token_required", "Pairing authorization is required");
    ...
    .get("/api/capabilities", (c) => c.json(agentdCapabilitiesSchema.parse({
      protocolVersion: 1,
      features: {
        tmuxSessions: true,
    ...
    .post("/internal/tmux-hook",
      (c, next) => {
        if (c.req.header("x-agentd-hook-token") !== deps.hookToken) throw new AgentdHttpError(401, "unauthorized", "Invalid tmux hook token");
        return next();
      },
      validate("form", hookFormSchema),
      (c) => {
        const form = c.req.valid("form") as z.infer<typeof hookFormSchema>;
        deps.application.hooks.handleTmux(form.event as AgentdHookEvent, form.client);
        return c.body(null, 204);
      })
...
export type { AgentdAuthContext, AgentdHttpDependencies, AgentdHttpStatus, AgentdHookEvent } from "./types.js";
export { agentdSocketReadyState, HonoSocketAdapter } from "./socket.js";
export type { AgentdSocket, AgentdSocketData } from "./socket.js";
export const agentdWebsocket: BunWebSocketHandler<BunWebSocketData> & { idleTimeout: number } = { ...websocket, idleTimeout: 0 };

function requireBearer(header: string | undefined, deps: AgentdHttpDependencies): AgentdAuthContext {
  if (!header?.startsWith("Bearer ")) throw new AgentdHttpError(401, "unauthorized", "Bearer authentication is required");
  const token = header.slice("Bearer ".length).trim();
  const context = deps.auth.authenticateAccessToken(token || undefined);
  if (!context) throw new AgentdHttpError(401, "unauthorized", "Bearer authentication is required");
  return context;
}

function mapError(error: unknown): AgentdHttpError {
  if (error instanceof AgentdHttpError) return error;
  if (error instanceof z.ZodError) return new AgentdHttpError(400, "invalid_request", "Request validation failed");
  if (isRecord(error) && typeof error.code === "string" && typeof error.message === "string") {
    const status = errorStatus(error.code, error.status);
    const details = isRecord(error.details) ? error.details : undefined;
    return new AgentdHttpError(status, error.code, error.message, details);
  }
  return new AgentdHttpError(503, "agentd_unavailable", "agentd could not complete the request");
}

function errorStatus(code: string, status: unknown): AgentdHttpStatus {
  if (isAgentdHttpStatus(status)) return status;
...
function errorBody(error: AgentdHttpError): { error: string; message: string; details?: Record<string, unknown> } {
  return { error: error.code, message: error.message, ...(error.details ? { details: error.details } : {}) };
}

function isAgentdHttpStatus(value: unknown): value is AgentdHttpStatus {
