...
import type { MuximodAuthContext } from "@muximo/muximod-http";
import type { MuximodSocket } from "@muximo/application";
import {
  canonicalPublicJwk,
  decodeBase64Url,
  encodePairingCode,
...
export type { PairingClaimNotification } from "@muximo/protocol";
...
type TrackedSockets = {
  ...
  expiryTimers: Map<MuximodSocket, NodeJS.Timeout>;
};
...
export class AuthService {
  ...
  public createPairing(overrides: Partial<Pick<AuthServiceOptions, "muximodBaseUrl">> = {}): PairingQrPayload {
  ...
  public trackSocket(context: AuthContext, socket: MuximodSocket): void {
    const tracked = this.sockets.get(context.sessionId) ?? {
      deviceId: context.deviceId,
      sockets: new Set<MuximodSocket>(),
      expiryTimers: new Map<MuximodSocket, NodeJS.Timeout>(),
    };
