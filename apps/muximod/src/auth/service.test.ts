...
import { createAgentDatabase, AuthStore } from "@muximo/persistence";
import { pairingClaimMessage, publicKeyFingerprint, sessionMessage, sha256Hex, signEcdsa } from "@muximo/protocol";
...
const authFixture = async (): Promise<FixtureHandle<AuthFixture>> => {
  ...
  const auth = new AuthService({ store: new AuthStore(database.sqlite), muximodBaseUrl: "http://127.0.0.1:4317" });
  ...
  execute: async (fixture, steps) => {
    for (const step of steps) {
      ...
      if (step.type === "consume-ticket") {
        const context = fixture.auth.authenticateAccessToken(fixture.session!.accessToken);
        const ticket = fixture.auth.issueWebSocketTicket(context!, "events");
        fixture.ticketSessionId = fixture.auth.consumeWebSocketTicket(ticket.ticket, "events")?.sessionId ?? null;
        fixture.ticketSecondUse = fixture.auth.consumeWebSocketTicket(ticket.ticket, "events");
      }
    }
  },
