...
describe("operation table support", () => {
  ...
  const defaultTable: OperationTable<
    Fixture,
    FixtureKey,
    {},
    string,
  ...
  const overrideTable: OperationTable<
    Fixture,
    FixtureKey,
    {},
    string,
  ...
  const errorTable: OperationTable<
    Fixture,
    FixtureKey,
    {},
    string,
  ...
  const aggregateCases = [
    {
      ...
      input: {},
      assert: [
        hasObserved<AggregateContext, TableAssertionError>(
          "assertionFailureNames",
          ["first", "second"],
        ),
        hasObserved<AggregateContext, TableAssertionError>(
          "assertionMessage",
          "aggregate: 2 assertion(s) failed",
        ),
      ],
    },
  ...
  const aggregateTable: OperationTable<
    Fixture,
    FixtureKey,
    {},
    TableAssertionError,
  ...
  const runnerProbeCases = [
    {
      ...
      assert: [
        hasObserved<RunnerProbeContext, RunnerProbeResult>("errorName", "TableAssertionError"),
      ],
    },
    {
  ...
  const runnerProbeTable: OperationTable<RunnerProbeFixture, "default", RunnerProbeInput, RunnerProbeResult, RunnerProbeContext> = {
    defaultFixture: () => ({ fixture: { events: [], error: null } }),
    cases: runnerProbeCases,
    execute: async (fixture, input) => {
      const handlers: Array<() => Promise<void>> = [];
      const capture: TestRegistrar = (_name, handler) => {
        handlers.push(async () => { await handler(); });
      };

      if (input === "unexpected-outcome") {
        runOperationTable(capture, {
          defaultFixture: () => ({ fixture: undefined }),
          cases: [{ name: "nested unexpected", input: {}, assert: [{ name: "observation only", check: () => undefined }] }],
          execute: () => { throw new Error("unexpected execute error"); },
          observe: () => ({}),
        });
      } else {
        runOperationTable(capture, {
          defaultFixture: (registerCleanup) => {
            registerCleanup?.(() => { fixture.events.push("first"); });
            registerCleanup?.(() => { fixture.events.push("second"); throw new Error("second cleanup"); });
            if (input === "setup-failure") throw new Error("fixture setup failed");
            return { fixture: undefined, cleanup: () => { fixture.events.push("returned"); } };
          },
