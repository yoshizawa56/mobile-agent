...
import { hasError, hasObserved, noFixture, returns, runOperationTable, type FixtureHandle, type OperationCase, type OperationTable, type TestRegistrar } from "@mobile-agent/test-support";
...
const pasteTable: OperationTable<PasteFixture, PasteFixtureKey, PasteInput, PasteResult, PasteContext> = {
  ...
  observe: (fixture, result) => {
    ...
    return {
      ...
      bufferNamePattern: setCall ? /^agentd-paste-[0-9a-f]{12}$/.test(setCall.name) : false,
