  ...
  return (
    <main className="flex h-[var(--app-viewport-height)] min-h-[var(--app-viewport-height)] flex-col overflow-x-hidden overflow-y-auto bg-flow-grid bg-[length:auto,32px_32px,32px_32px,auto] text-ink">
      ...
      <section className="mx-auto w-full max-w-[570px] flex-1 px-6 py-[58px] max-[620px]:px-[max(14px,var(--safe-area-right))] max-[620px]:pb-[calc(30px+var(--safe-area-bottom))] max-[620px]:pt-[38px]">
        ...
        {viewModel.isPairingQr ? <div className="mb-4 rounded-xl border border-[#5bd6b2]/35 bg-[#1c5b4c]/28 p-[0.85rem_1rem] text-[#b9f5de]" role="status">{viewModel.pairingMessage ?? "Pairing…"}</div> : null}
        ...
        {!viewModel.isScanningQr && !viewModel.isPairingQr ? (
          <button className="mb-4 flex min-h-[45px] w-full items-center justify-between gap-3 rounded-[9px] border border-[#4a9a57] bg-lime px-[15px] text-[0.71rem] font-bold text-[#061008] transition-colors hover:bg-[#b0ffb8] max-[920px]:min-h-[52px] max-[920px]:text-[0.88rem]" type="button" onClick={viewModel.onOpenQrScanner}>
