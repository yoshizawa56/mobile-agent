...
export function ToastPattern({ agents, durationMs = 5_000, onOpen }: { agents: WaitingAgent[]; durationMs?: number; onOpen?: (agent: WaitingAgent) => void }) {
  return (
...
function ToastCard({ agent, index, durationMs, onOpen }: { agent: WaitingAgent; index: number; durationMs: number; onOpen?: (agent: WaitingAgent) => void }) {
  ...
  return (
    ...
    >
      <span className="notice-a-timer-v absolute inset-y-0 left-0 w-[3px] bg-amber shadow-[0_0_12px_rgb(241_199_109_/_60%)]" style={{ animationDelay: `${delayMs}ms`, animationDuration: `${durationMs}ms` }} />
      <span className="flex items-center gap-2.5 py-2.5 pl-[15px] pr-2">
        <span className={`grid size-8 shrink-0 place-items-center rounded-lg border font-mono text-[0.72rem] font-extrabold ${agent.badgeClass}`}>{agent.monogram}</span>
        <span className="flex min-w-0 flex-1 flex-col gap-[3px]">
          <span className="flex min-w-0 items-center gap-1.5">
            <strong className="min-w-0 overflow-hidden text-[0.7rem] font-bold text-ellipsis whitespace-nowrap text-[#f3e7c3]">{agent.name}</strong>
            <span className="shrink-0 rounded-[4px] bg-amber px-[5px] py-[2px] font-mono text-[0.48rem] font-bold leading-none text-[#1a1506]">{agent.state === "waiting_input" ? "INPUT" : "APPROVAL"}</span>
          </span>
          <small className="overflow-hidden font-mono text-[0.54rem] text-ellipsis whitespace-nowrap text-[#d9b36b]">{agent.recentOutput}</small>
        </span>
      ...
      {expanded ? (
        <div className="notice-b pointer-events-auto w-full max-w-[330px] overflow-hidden rounded-[15px] border border-[#4c3d1c] bg-[rgb(20_17_8_/_95%)] shadow-[0_18px_54px_rgb(0_0_0_/_52%),inset_0_0_0_1px_rgb(241_199_109_/_8%)] backdrop-blur-[14px]">
          <header className="flex items-center justify-between gap-2 border-b border-[#3a2f15] px-3 py-2">
            ...
            <button className="grid size-6 place-items-center rounded-md text-[0.62rem] text-[#8a7442] transition-colors hover:text-amber" type="button" onClick={() => setExpanded(false)} aria-label="Collapse">▾</button>
          </header>
          <div className="flex flex-col">
            {agents.map((agent) => (
              <div key={agent.id} className="flex items-center gap-2.5 border-b border-[#2c2410] px-3 py-2.5 last:border-0">
                <span className={`grid size-7 shrink-0 place-items-center rounded-lg border font-mono text-[0.64rem] font-extrabold ${agent.badgeClass}`}>{agent.monogram}</span>
                <div className="flex min-w-0 flex-1 flex-col gap-[2px]">
                  <strong className="min-w-0 overflow-hidden text-[0.66rem] font-bold text-ellipsis whitespace-nowrap text-[#f3e7c3]">{agent.name}</strong>
                  <small className="overflow-hidden font-mono text-[0.52rem] text-ellipsis whitespace-nowrap text-[#d9b36b]">{agent.stateLabel} · {agent.time}</small>
                </div>
                <button className="shrink-0 rounded-md border border-[#5a4a24] bg-[rgb(38_30_12_/_70%)] px-2 py-[5px] font-mono text-[0.56rem] font-bold text-[#f1d9a0] transition-colors hover:border-amber hover:text-amber" type="button" aria-label={`Open ${agent.name}`}>open →</button>
              </div>
            ))}
          </div>
        </div>
      ) : (
        <button className="notice-b pointer-events-auto flex items-center gap-2 rounded-full border border-[#4c3d1c] bg-[rgb(20_17_8_/_95%)] py-[7px] pl-[9px] pr-3.5 shadow-[0_16px_44px_rgb(0_0_0_/_50%)] backdrop-blur-[14px]" type="button" onClick={() => setExpanded(true)} aria-expanded={false} aria-label={`${agents.length} agents waiting`}>
          <span className="flex items-center -space-x-[6px]">
            {agents.slice(0, 3).map((agent) => (
              <span key={agent.id} className={`grid size-[22px] place-items-center rounded-full border-2 border-[rgb(20_17_8_/_95%)] font-mono text-[0.5rem] font-extrabold ${agent.badgeClass}`}>{agent.monogram}</span>
            ))}
          </span>
...
export function BannerPattern({ agents }: { agents: WaitingAgent[] }) {
  return (
    <div className="pointer-events-none absolute inset-x-0 top-0 z-30 overflow-hidden">
      {agents.map((agent, index) => (
        <div key={agent.id} className="notice-c pointer-events-auto relative flex items-center gap-2.5 border-b border-[#3d3115] bg-[rgb(22_17_6_/_96%)] py-[9px] pl-[13px] pr-3 shadow-[0_14px_34px_rgb(0_0_0_/_38%)] backdrop-blur-[10px]" style={{ animationDelay: `${index * 160}ms` }}>
          ...
          <span className={`grid size-[22px] shrink-0 place-items-center rounded-md border font-mono text-[0.56rem] font-extrabold ${agent.badgeClass}`}>{agent.monogram}</span>
          <div className="flex min-w-0 flex-1 flex-col gap-[1px]">
            <strong className="min-w-0 overflow-hidden text-[0.66rem] font-bold text-ellipsis whitespace-nowrap text-[#f3e7c3]">{agent.name}</strong>
            <small className="overflow-hidden font-mono text-[0.52rem] text-ellipsis whitespace-nowrap text-[#d9b36b]">{agent.stateLabel}</small>
          </div>
