...
import type { AgentdApplication, AgentdSocket } from "@mobile-agent/application";
import {
  runOperationTable,
  hasObserved,
  type Assertion,
...
import type { AgentdApp } from "./app.js";
import { agentdWebsocket, createAgentdApp } from "./app.js";
import type { AgentdAuthPort } from "./types.js";
...
type SocketInput =
  | { kind: "plain"; endpoint: "terminal" | "events" }
  | { kind: "websocket"; endpoint: "terminal" | "events"; ticket: string; payload?: readonly number[] };
...
const fixture = (): FixtureHandle<SocketFixture> => {
  ...
  const validTickets = new Set(["ticket-terminal", "ticket-events"]);
  ...
  const application: AgentdApplication = {
  ...
  const echo = (socket: AgentdSocket): void => {
...
const cases = [
  {
    ...
    input: { kind: "plain", endpoint: "terminal" },
    assert: [
  ...
  },
  {
    ...
    input: { kind: "websocket", endpoint: "terminal", ticket: "invalid-ticket" },
    assert: [
  ...
  },
  {
    ...
    input: { kind: "websocket", endpoint: "terminal", ticket: "ticket-terminal", payload: [0, 1, 255] },
    assert: [
  ...
  {
    name: "opens the events endpoint through the same authenticated upgrade",
    input: { kind: "websocket", endpoint: "events", ticket: "ticket-events" },
    assert: [
      websocketIs({ opened: true, received: [] }),
      hasObserved<SocketContext, SocketResult>("consumedTickets", ["events:ticket-events"]),
      hasObserved<SocketContext, SocketResult>("eventConnections", 1),
    ],
  },
...
const table: OperationTable<SocketFixture, "default", SocketInput, SocketResult, SocketContext> = {
  ...
  execute: async (world, input) => {
    const url = `http://127.0.0.1:${world.server.port}/${input.endpoint}`;
    if (input.kind === "plain") {
  ...
  observe: (world) => ({
    ...
    eventConnections: world.eventConnections,
    idleTimeout: agentdWebsocket.idleTimeout,
  }),
};

describe("agentd Bun WebSocket boundary", () => {
