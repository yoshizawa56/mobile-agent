...
const queryCases = [
  {
    ...
    assert: [
      ...
      hasObserved<QueryContext, unknown>("requests", ["http://muximod.local/api/sessions"]),
    ],
  },
  {
    ...
    assert: [
      ...
      hasObserved<QueryContext, unknown>("requests", ["http://muximod.local/api/panes?session=integration"]),
    ],
  },
  {
    ...
    input: {
      ...
      response: { workspaces: [{ id: "workspace-1", name: "muximo", directory: "~/work/muximo", isGit: true, setupScriptPath: null, cleanupScriptPath: null, worktreeCopyPatterns: [] }] },
    },
    assert: [
      returns<QueryContext, unknown>([{ id: "workspace-1", name: "muximo", directory: "~/work/muximo", isGit: true, setupScriptPath: null, cleanupScriptPath: null, worktreeCopyPatterns: [] }]),
      hasObserved<QueryContext, unknown>("requests", ["http://muximod.local/api/workspaces"]),
    ],
  },
  {
    ...
    input: {
      ...
      response: { directories: [{ id: "workspace-1", name: "muximo", directory: "~/work/muximo", isGit: true, setupScriptPath: null, cleanupScriptPath: null, worktreeCopyPatterns: [] }] },
    },
    assert: [
      returns<QueryContext, unknown>([{ id: "workspace-1", name: "muximo", directory: "~/work/muximo", isGit: true, setupScriptPath: null, cleanupScriptPath: null, worktreeCopyPatterns: [] }]),
      hasObserved<QueryContext, unknown>("requests", ["http://muximod.local/api/workspace-directories?"]),
    ],
  },
  {
    name: "preserves structured directory errors from muximod",
    input: {
...
const mutationCases = [
  {
    ...
    input: {
      ...
      input: { name: "renamed" },
      response: { id: "workspace-1", name: "renamed", directory: "/work/muximo", isGit: true, setupScriptPath: null, cleanupScriptPath: null, worktreeCopyPatterns: [] },
    },
    assert: [
      returns<MutationContext, unknown>({ id: "workspace-1", name: "renamed", directory: "/work/muximo", isGit: true, setupScriptPath: null, cleanupScriptPath: null, worktreeCopyPatterns: [] }),
      hasObserved<MutationContext, unknown>("requests", [{ method: "PATCH", url: "http://muximod.local/api/workspaces/workspace-1", body: { name: "renamed" } }]),
    ],
  },
  {
    ...
    assert: [
      ...
      hasObserved<MutationContext, unknown>("requests", [{ method: "DELETE", url: "http://muximod.local/api/workspaces/workspace-1", body: null }]),
    ],
  },
...
const routeCases = [
  {
    ...
    assert: [returns<{}, MuximodConnection>({ httpBaseUrl: "https://workstation.tailnet.ts.net", websocketUrl: "wss://workstation.tailnet.ts.net/terminal", eventsWebsocketUrl: "wss://workstation.tailnet.ts.net/events", route: "serve" })],
  },
  {
    name: "preserves a reverse proxy path",
    input: { kind: "serve", url: "https://example.test/muximod/" },
    assert: [returns<{}, MuximodConnection>({ httpBaseUrl: "https://example.test/muximod", websocketUrl: "wss://example.test/muximod/terminal", eventsWebsocketUrl: "wss://example.test/muximod/events", route: "serve" })],
  },
  {
    name: "builds a same-origin development route",
    input: { kind: "same-origin", url: "http://localhost:5173" },
    assert: [returns<{}, MuximodConnection>({ httpBaseUrl: "http://localhost:5173", websocketUrl: "ws://localhost:5173/terminal", eventsWebsocketUrl: "ws://localhost:5173/events", route: "same-origin" })],
  },
  {
