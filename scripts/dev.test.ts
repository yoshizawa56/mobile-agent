...
const pureCases = [
  ...
  },
  {
    ...
    assert: [succeeds("deduplicates process owners", (value) => {
      assert.deepEqual(value.owners, [{ pid: "123", command: "node" }, { pid: "456", command: "vite" }]);
  ...
  },
  {
    ...
    assert: [succeeds("configures the expected Serve endpoint", (value) => {
      assert.deepEqual(value.calls, [{ command: "tailscale-test", args: ["serve", "--bg", "--https=443", "--yes", "http://127.0.0.1:15227"] }]);
  ...
  },
  {
    ...
    assert: [succeeds("converts the WebSocket URL to HTTP", (value) => {
...
const supervisorCases = [
  ...
  },
  {
    ...
    fixture: "default",
    steps: [{ type: "start" }, { type: "replace-web" }, { type: "stop", reason: "test", exitCode: 0 }, { type: "wait" }],
    assert: [observes("preserves the replacement listener", (ctx) => {
...
const supervisorTable = {
  ...
  execute: async (fixture, steps) => {
    ...
    for (const step of steps) {
      ...
      if (step.type === "replace-web") fixture.runtime.ports.set("web", { healthy: true, owners: [{ pid: "999", command: "replacement" }] });
