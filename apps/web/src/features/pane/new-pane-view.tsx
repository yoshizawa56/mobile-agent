...
const placementCardBase = "flex min-w-0 cursor-pointer flex-col items-center justify-center gap-[6px] rounded-lg border border-[#1e4828] bg-[rgb(7_24_11_/_70%)] p-2 text-[#82b488] transition-colors max-[920px]:min-h-[62px]";
const placementCardSelected = "border-[#3d8b4c] bg-[rgb(13_40_19_/_82%)] text-[#c3f4c9] shadow-[inset_0_-2px_0_var(--color-lime-deep)]";
...
const agentOptions: { value: NewPaneAgent; label: string; monogram: string; badgeClass: string }[] = [
  { value: "codex", label: "Codex", monogram: "C", badgeClass: "border-[#2b6f8a] bg-[rgb(21_94_117_/_24%)] text-[#7dd3fc]" },
  { value: "claude", label: "Claude", monogram: "C", badgeClass: "border-[#9a5b3c] bg-[rgb(154_52_18_/_22%)] text-[#fdba74]" },
  { value: "opencode", label: "OpenCode", monogram: "O", badgeClass: "border-[#3d8b4c] bg-[rgb(57_214_91_/_14%)] text-lime" },
];
  ...
  return (
    <main className="flex h-[var(--app-viewport-height)] min-h-[var(--app-viewport-height)] flex-col overflow-x-hidden overflow-y-auto bg-flow-grid bg-[length:auto,32px_32px,32px_32px,auto] text-ink">
      ...
      <section className="mx-auto w-full max-w-[570px] flex-1 px-6 py-[52px] max-[620px]:px-[max(14px,var(--safe-area-right))] max-[620px]:pb-[calc(32px+var(--safe-area-bottom))] max-[620px]:pt-8">
        ...
        <form id="new-pane-form" className="flex flex-col gap-[17px]" onSubmit={(event) => { event.preventDefault(); if (canCreate) viewModel.onCreate(); }}>
          ...
          <fieldset className="mt-0.5 flex flex-col gap-2 border-0 p-0">
            ...
            <div className="grid grid-cols-3 gap-[7px]">
              {placementOptions.map(({ value, icon, label }) => {
                ...
                return (
                  <label key={value} className={`${placementCardBase} ${selected ? placementCardSelected : ""} ${disabled ? placementCardDisabled : ""}`}>
                    <input type="radio" className="sr-only" name="pane-placement" checked={selected} onChange={() => viewModel.onPlacementChange(value)} disabled={disabled} />
            ...
            <small className="font-mono text-[0.52rem] leading-[1.45] text-[#5d9168] max-[620px]:text-[0.72rem]">{placementDescription[viewModel.placement]}</small>
          </fieldset>
                ...
                {agentOptions.map(({ value, label, monogram, badgeClass }) => {
                  ...
                  return (
                    <label key={value} className={`${placementCardBase} ${selected ? placementCardSelected : ""}`}>
                      <input type="radio" className="sr-only" name="pane-agent" checked={selected} onChange={() => viewModel.onAgentChange(value)} />
                      <span className={`grid size-[21px] place-items-center rounded-[6px] border font-mono text-[0.68rem] font-bold ${badgeClass}`} aria-hidden="true">{monogram}</span>
                      <strong className="text-[0.62rem] font-semibold leading-none text-[#c3f4c9]">{label}</strong>
                    </label>
                  );
                })}
      ...
      <footer className="sticky bottom-0 z-10 mt-auto shrink-0 border-t border-[#17391f] bg-[#030a05]/80 px-7 py-3 backdrop-blur-[16px] max-[620px]:px-[max(14px,var(--safe-area-right))] max-[620px]:pb-[calc(12px+var(--safe-area-bottom))]">
        <button className="mx-auto flex min-h-[45px] w-full max-w-[570px] items-center justify-between gap-3 rounded-[9px] border border-[#4a9a57] bg-lime px-[15px] text-[0.71rem] font-bold text-[#061008] transition-colors hover:bg-[#b0ffb8] disabled:cursor-not-allowed disabled:opacity-35 max-[920px]:min-h-[52px] max-[920px]:text-[0.88rem]" type="submit" form="new-pane-form" disabled={!canCreate || viewModel.isCreating}>{viewModel.isCreating ? "Opening pane…" : "Open pane"}<span className="text-[1.1rem]">{viewModel.isCreating ? "…" : "→"}</span></button>
      </footer>
    </main>
  );
}
