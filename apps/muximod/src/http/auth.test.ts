...
import { AuthStore, createAgentDatabase } from "@muximo/persistence";
import { AuthService } from "../auth/service.js";
import { createMuximodApp, type MuximodHttpDependencies } from "./app.js";

type HttpStep = { type: "health" | "info" | "protected" | "preflight" };
type HttpFixture = {
  database: ReturnType<typeof createAgentDatabase>;
  app: ReturnType<typeof createMuximodApp>;
...
const httpFixture = (): FixtureHandle<HttpFixture> => {
  ...
  const auth = new AuthService({ store: new AuthStore(database.sqlite), muximodBaseUrl: "http://muximod.example" });
  const app = createMuximodApp({
    ...
    hookToken: "hook",
  } satisfies MuximodHttpDependencies);
...
const table: ScenarioTable<HttpFixture, "default", HttpStep, undefined, HttpContext> = {
  ...
  execute: async (fixture, steps) => {
    for (const step of steps) {
      ...
      if (step.type === "health") response = await fixture.app.request("http://muximod.example/health");
      else if (step.type === "info") response = await fixture.app.request(new Request("http://muximod.example/auth/v1/info", { headers: { origin: "http://web.example" } }));
      else if (step.type === "protected") response = await fixture.app.request("http://muximod.example/api/capabilities");
      else response = await fixture.app.request(new Request("http://muximod.example/api/capabilities", { method: "OPTIONS", headers: { origin: "http://web.example", "access-control-request-method": "GET" } }));
...
describe("muximod HTTP authentication boundary", () => {
