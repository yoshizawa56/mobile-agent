...
const fixtureFactories: Readonly<Record<AgentFixtureKey, () => Promise<FixtureHandle<AgentFixture>>>> = {
  ...
  remote: async () => {
    ...
    fixture.env = { ...fixture.env, AGENT_CODEX_BIN: fakeCodex, AGENT_CODEX_NAME_BIN: fakeName, CODEX_HOME: join(fixture.root, "codex-home"), TEST_AGENT_NAME_LOG: fixture.nameLog, TEST_AGENT_REMOTE_LOG: fixture.remoteLog };
    return toHandle(fixture);
  },
  missing: async () => {
    const fixture = createFixture();
...
const agentTable: ScenarioTable<AgentFixture, AgentFixtureKey, AgentStep, AgentResult, AgentContext> = {
  ...
  execute: async (fixture, steps) => {
    if (steps.length !== 1) throw new Error("agent command scenario requires exactly one action step");
    const step = steps[0]!;
    switch (step.type) {
...
const managedSessionAssertion: Assertion<ExtendedContext, ExtendedResult> = {
  ...
  check: (ctx) => {
    ...
    expect(ctx.created?.command).toContain("'/opt/mobile-agent/agent' shell");
    expect(ctx.options).toEqual([{ name: "work", key: "default-command", value: "'/opt/mobile-agent/agent' shell" }]);
...
const extendedFixtureFactories: Readonly<Record<ExtendedFixtureKey, () => Promise<FixtureHandle<ExtendedFixture>>>> = {
  "managed-session": async () => {
    ...
    fixture.env = { ...fixture.env, AGENTD_AGENT_COMMAND: "/opt/mobile-agent/agent" };
  ...
  "log-level": async () => {
    const fixture = createExtendedFixture();
    fixture.env = { ...fixture.env, AGENT_LOG_LEVEL: "debug" };
    return toExtendedHandle(fixture);
  },
  diagnostics: async () => {
    const fixture = createExtendedFixture();
...
const extendedTable: ScenarioTable<ExtendedFixture, ExtendedFixtureKey, ExtendedStep, ExtendedResult, ExtendedContext> = {
  ...
  cases: extendedCases,
  execute: async (fixture, steps) => {
    if (steps.length !== 1) throw new Error("extended agent command scenario requires exactly one action step");
    const step = steps[0]!;
    switch (step.type) {
...
function assertNever(value: never): never {
  throw new Error(`unhandled agent command step: ${String(value)}`);
}
...
async function executeInterrupt(fixture: AgentFixture): Promise<AgentResult> {
  ...
  const list = await runCommand(fixture, ["list", "--json"], { ...fixture.env, AGENT_ASSUME_YES: "1", TEST_AGENT_EXIT_STATUS: "0" });
...
async function executeRemote(fixture: AgentFixture): Promise<AgentResult> {
  ...
  fixture.env = { ...fixture.env, AGENT_ASSUME_YES: "1" };
...
async function runCommand(fixture: AgentFixture, args: string[], environment = fixture.env): Promise<{ code?: number; output: string; error?: unknown }> {
  ...
  const command = new AgentCommand({ cwd: fixture.workspace, databaseFile: fixture.database, env: environment, io: { out: output, err: output } });
  try {
