...
const parseCases = [
  {
    name: "uses an agentd-only Serve profile by default",
    input: { args: ["tailscale"], environment: { AGENTD_PORT: "4391", AGENT_SERVE_PORT: "8444", TAILSCALE_BIN: "tailscale-test" } },
    assert: [matchesOptions({ provider: "tailscale", agentdHost: "127.0.0.1", agentdPort: 4391, externalPort: 8444, tailscaleBinary: "tailscale-test" })],
  },
  {
    name: "parses the managed agentd logging options",
    input: { args: ["tailscale", "--log-level", "debug", "--log-file", "/private/tmp/mobile-agent-serve.log"], environment: {} },
    assert: [matchesOptions({ logLevel: "debug", logFile: "/private/tmp/mobile-agent-serve.log" })],
  },
  {
...
const runCases = [
  {
    name: "ensures agentd and upserts the fixed Tailscale endpoint",
    input: { args: ["tailscale", "--port", "443"], environment: { AGENTD_PORT: "4391", AGENT_TAILSCALE_HOSTNAME: "agent-host.tailnet.ts.net" } },
    assert: [
      returns<RunContext, number>(0),
      hasObserved<RunContext, number>("ensured", [expect.objectContaining({ agentdPort: 4391, externalPort: 443 })]),
      hasObserved<RunContext, number>("calls", [{ command: "tailscale", args: ["serve", "--bg", "--https=443", "--yes", "http://127.0.0.1:4391"] }]),
      hasObserved<RunContext, number>("summary", "agent serve tailscale: https://agent-host.tailnet.ts.net/ -> http://127.0.0.1:4391"),
    ],
  },
  {
...
const runTable: OperationTable<RunFixture, "default", RunInput, number, RunContext> = {
  ...
  observe: (fixture) => ({
    ...
    error: fixture.error,
    helpBackground: fixture.output.includes("Ensures agentd is running in the background"),
