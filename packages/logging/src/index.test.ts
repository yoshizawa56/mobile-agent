...
const cases = [
  { name: "adds process metadata and immutable child context", fixture: "metadata", steps: [{ type: "metadata" }], assert: [hasNoError<LogContext, LogResult>(), metadataAssertion] },
  { name: "renders human output without a stack by default and includes it in verbose mode", fixture: "human", steps: [{ type: "human" }], assert: [hasNoError<LogContext, LogResult>(), humanAssertion] },
  { name: "writes background logs as JSON and rotates bounded files", fixture: "rotate", steps: [{ type: "rotate" }], assert: [hasNoError<LogContext, LogResult>(), rotateAssertion] },
  { name: "isolates synchronous and asynchronous stream failures", fixture: "stream", steps: [{ type: "stream" }], assert: [hasNoError<LogContext, LogResult>(), streamAssertion] },
  { name: "filters records by level and parses configured levels", fixture: "level", steps: [{ type: "level" }], assert: [hasNoError<LogContext, LogResult>(), levelAssertion] },
  { name: "keeps hostile errors and subprocess diagnostics safe", fixture: "hostile", steps: [{ type: "hostile" }], assert: [hasNoError<LogContext, LogResult>(), hostileAssertion] },
    ...
    switch (steps[0]!.type) {
      ...
      case "human": {
        const record: LogRecord = {
          ...
          context: {},
          fields: { message: "unexpected error: failed to connect", error: { name: "Error", message: "failed to connect", stack: "Error: failed to connect\n    at main" } },
        };
        fixture.output = formatHumanRecord(record) + "\n---verbose---\n" + formatHumanRecord(record, true);
        return { done: true };
      }
      case "rotate":
        for (let index = 0; index < 8; index += 1) fixture.logger!.info("daemon.health_check", { attempt: index, ok: true });
        return { done: true };
      case "stream":
      ...
      case "level":
        ...
        return { done: true };
      case "hostile": {
        const hostile = new Proxy({}, { ownKeys() { throw new Error("ownKeys should not escape diagnostics"); } });
        fixture.fields = errorFields(hostile);
        const subprocessError = new Error("Command failed: backend --prompt sensitive prompt\\nsecret output");
        Object.defineProperty(subprocessError, "cause", { configurable: true, get() { throw new Error("cause accessor failed"); } });
        fixture.fields = errorFields(subprocessError);
        const throwingMessage = new Error("fallback");
        Object.defineProperty(throwingMessage, "message", { configurable: true, get() { throw new Error("message accessor failed"); } });
