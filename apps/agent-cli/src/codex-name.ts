...
const usage = "Usage: agent-codex-name --thread-id ID (--name NAME|--archive|--unarchive) [--socket PATH] [--transport auto|http|raw]\n";
...
try {
  ...
  process.stderr.write(`agent-codex-name: ${error instanceof Error ? error.message : String(error)}\n`);
