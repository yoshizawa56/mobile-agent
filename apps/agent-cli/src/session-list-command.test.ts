...
import { workspaceIdForPath } from "@mobile-agent/agentd/workspace-selection";
import {
  hasError,
  hasNoError,
  hasObserved,
...
const listTable: OperationTable<ListFixture, "default", ListInput, ListResult, ListContext> = {
  ...
  execute: async (fixture, input) => {
    const command = new AgentCommand({
      ...
      env: { ...process.env, AGENTD_DB_FILE: fixture.database },
...
describe("agent session list command", () => {
...
async function createListFixture(registerCleanup?: CleanupRegistrar): Promise<FixtureHandle<ListFixture>> {
  const root = mkdtempSync(join(tmpdir(), "mobile-agent-session-list-test-"));
